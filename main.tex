...
\end{nproblem}
Sort elements lexicographically based on the vector $(w_1(e),\dots,w_k(e))$ and use the greedy algorithm to find a max weight base $B^*$. Then check whether $B^*$ is the simultaneously maximal with weights $w_1,\dots,w_k$.
\begin{proof}
    We need to prove that if there exists a simultaneously maximal base, the algorithm can find it.
    $k=1$ case is trivial. Suppose the algorithm finds the simultaneously max base for $k$ weights. Assume that the algorithm generates $B$. Let $B^*$ be the simultaneously max base for which its intersection with $B$ is as large as possible. By the induction hypothesis, $B$ is a simultaneously max base for weights $w_1,\dots,w_{k}$. Let $f$ be the element with the largest weight $B-B^*$. \dots
\end{proof}
